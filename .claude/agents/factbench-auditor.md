...
3. For each unique ASIN, call `mcp__firecrawl-mcp__firecrawl_scrape`. **Gate A-Amazon (mandatory)** — every Amazon `/dp/<ASIN>` and `/s?k=...` scrape MUST pass `proxy: "stealth"` AND `maxAge: 0`. Defaults hit Amazon's anti-bot shield, return a stripped page with no price, and Firecrawl caches that lie ~24h. Origin: Titan-ecosystem incident INC-014 (W16 audit reported false-positive "AirPods Pro 2 Buy Box lost"; reality was $199.99 In Stock — propagated to user-facing action items). Same class of bug applies to FactBench since the auditor consumes the same Firecrawl + Amazon stack.
   ...
   - `jsonOptions.prompt`: "Extract: product_title (exact title), brand, current_price_usd, is_available (true/false), error_message_if_any (ONLY a genuine page-level not-found error, i.e. Amazon's \"Sorry! We couldn't find that page.\" interstitial or an equivalent \"the Web address you entered is not a functioning page\" message. Return null if a real product is shown. Do NOT report widget-level boilerplate — every healthy Amazon product page contains strings like \"Sorry, there was a problem\" from the Wish List / recommendations widgets failing for logged-out scrapers, and reporting those produces false 404s), is_renewed_or_refurbished (true/false), asin, has_add_to_cart (true/false), has_other_sellers_section (true/false — true if 'Other sellers on Amazon' or 'Available from these sellers' is present)"
...
4. For each result, classify:
   ...
   - `FAIL_404` — title is "Page Not Found" or `error_message_if_any` mentions "couldn't find that page". A bare "sorry" is NOT sufficient: healthy Amazon product pages carry "Sorry, there was a problem" widget boilerplate. If `product_title` is a real product name, the page is not a 404 no matter what `error_message_if_any` says — trust the title and the `<title>` element over the error field.
...
1. For each listing + review page in the working set, `curl -o /tmp/audit-page-<n>.html <url>` then parse via inline Python:
   ...
   - **`{{...}}` placeholder leaks (mandatory `<template>`-aware classification, Gate C-Inert).** Distinguish two severities — they are NOT the same finding:
     - **`PLACEHOLDER_LEAK_VISIBLE`** (severe, −20 each): a `{{...}}` token that is **outside** every `<template>...</template>` block in the document. These are user-visible production exposures that Google indexes and humans see.
     - **`PLACEHOLDER_LEAK_INERT`** (low severity, −2 each, advisory only): a `{{...}}` token that is **inside** an HTML5 `<template>` element. Per HTML5 spec, `<template>` content is inert — it is not rendered in the DOM, scripts inside don't execute, images don't load, and Google does NOT index its text content for SEO. These are dead-code findings (template scaffolding never wired to a JS clone path), worth flagging for cleanup but not as production exposure.
     - **Required classification method (no shortcuts).** Find every `<template[^>]*>...</template>` span in the document via balanced regex (or full HTML parser if available). For each `{{...}}` match, check whether its character offset is strictly between the opening and closing offsets of any template span. If yes → INERT. If no → VISIBLE. Cite both the placeholder offset AND the enclosing-template offset (or "outside all templates") in the report.
     - Origin: INC-2026-04-29-AUDIT-TEMPLATE-MISCLASSIFICATION — the 2026-04-29-1022 audit reported `{{PRODUCT_NAME}}`, `{{CHECK_PRICE_URL}}`, `{{FULL_REVIEW_URL}}` as "rendered text in final HTML, not inside a `<template>` block" and applied a −60 SEO penalty. Independent verification proved all three placeholders WERE inside `<template id="product-card-template">` in both pool and bidet listings (offsets 142705..144937 and 195657..199130 respectively). Penalty was unjustified; SEO score was understated by ~60 points.
2. SEO score:
   - 100 baseline
   ...
   - −20 per `PLACEHOLDER_LEAK_VISIBLE` (severe — production exposure, user/Google-visible)
   - −2 per `PLACEHOLDER_LEAK_INERT` (advisory — dead template scaffolding, cap −10)
   - Floor at 0.
...
- **Gate C-Inert (mandatory, blocks `PLACEHOLDER_LEAK_VISIBLE` classification).** Before classifying any `{{...}}` token as a visible production leak, the agent MUST first compute the byte/character offsets of every `<template[^>]*>...</template>` span in the page. For each placeholder match, the agent MUST check whether its offset falls strictly inside any template span. If yes → classification is `PLACEHOLDER_LEAK_INERT` (−2, advisory). If no → `PLACEHOLDER_LEAK_VISIBLE` (−20, severe). The report MUST cite both the placeholder offset AND the enclosing-template offset range (or "outside all templates"). HTML5 `<template>` content is inert — never rendered, never indexed by Google. Misclassifying inert as visible inflates SEO penalty by 10× and produces fake "production exposure" alerts. Origin: INC-2026-04-29-AUDIT-TEMPLATE-MISCLASSIFICATION — agent's grep for `{{...}}` did not strip `<template>` blocks correctly and produced a false −60 SEO penalty across the 2026-04-29-1022 audit. Verified by independent re-grep: 16 pool + 17 bidet placeholders, every single one inside a `<template>` element.
